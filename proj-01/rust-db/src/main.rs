...
INSERT id = \"lib3\", title = \"Cubical Type Theory Notes\", year = 2020, pages = 210, rating = 4.5, topic = \"Cubical\", is_foundational = false INTO library
...
SELECT name, introduced_by FROM concepts WHERE is_univalent = true
SELECT title, is_foundational FROM library

DELETE \"lib2\" FROM library
SELECT id, title FROM library WHERE pages > 250
